...
- [`web/streamlit_app_v6.py`](web/streamlit_app_v6.py) — the **V6 web app** (model, scenarios, model families, statistics reports, Dashboards, a global log filter and activity renaming, **plus a pre-mining cost screen**), hosted at https://pm4py-ucm.streamlit.app/ — click, don't code: upload an XES/CSV, tune the miner, download the result. Every earlier app (V5, V3, V2) is **deprecated** — see [Web front-end](#web-front-end). (The frozen **V2** scenarios app stays at https://pm4py-ucm-scenarios.streamlit.app/ for a paper under review.)
...
The [Streamlit](https://streamlit.io) front-end — now the **V6** app
([`web/streamlit_app_v6.py`](web/streamlit_app_v6.py)) — serves the
deployment: a left-rail workspace over these views (a strict superset of
the deprecated V5 and four-tab V3 apps), with a global **log filter** and
**activity renaming** applied before mining across every view, and a
**cost screen** that sizes a log before the miner is started:
...
deployment's main file, is a shim that runs V6, so that URL always
tracks the latest code).

**Every app older than V6 is deprecated.** They are kept runnable so
older results stay reproducible, and each says so when you start it, but
they receive no new features — including settings added to the session
registry, which they save but do not restore. Point new work at V6.

| App | File | Status |
|---|---|---|
| **V6** | [`streamlit_app_v6.py`](web/streamlit_app_v6.py) | **Current.** Deployed at https://pm4py-ucm.streamlit.app/ |
| V5 | [`streamlit_app_v5.py`](web/streamlit_app_v5.py) | Deprecated — workspace shell + Dashboards, no cost screen, no Simulation |
| V3 | [`streamlit_app_v3.py`](web/streamlit_app_v3.py) | Deprecated — the earlier four-tab app |
| V2 | [`streamlit_app_v2.py`](web/streamlit_app_v2.py) | Deprecated **and frozen** — see below; do not edit |
...
must stay as published. That is stricter than deprecation: V2 may not be
changed at all, which is also why it is the one app that renders no
deprecation notice — a banner would change what the reviewers see.
...
# Older apps still run, and say they are deprecated when they start:
# streamlit run web/streamlit_app_v5.py  # V5 — deprecated
# streamlit run web/streamlit_app_v3.py  # V3 — deprecated
# streamlit run web/streamlit_app_v2.py  # V2 — deprecated + frozen
```
...
overwhelming. The optional `decomposition=` keyword splits the result into a
**root map plus plug-in (sub-)maps connected by Stubs**. Four public entry
points take it, and all four interpret it identically:
`discover_ucm_inductive`, `convert_to_ucm`, `discover_scenarios` and
`discover_ucm_family`. The same
...
| `"auto"`       | All four boundary rules on; `max_leaves_per_map` and `min_leaves_to_decompose` **fitted to the tree shape** at conversion time — see `suggest_decomposition`. |
...
Anything else — another string, another type — raises `ValueError`.

`pm4py_ucm.suggest_decomposition(tree)` returns the shape-fitted dict
directly if you want to inspect or tweak it. It scales the two size knobs
with the tree's activity-leaf count `N` rather than fixing magic numbers:

- `max_leaves_per_map = clamp(round(k·√N), 6, 40)` — **sub-linear**, so a big
  tree does not simply get proportionally more maps. `k` is 1.5 by default
  and 1.0 for `level="fine"`, which is the only other level.
- `min_leaves_to_decompose = clamp(round(0.15·N), 3, 12)`, and never above
  `max_leaves_per_map - 1`. This floor, not the cap, is what actually
  controls fragmentation: a large tree gets bigger plug-ins instead of
  dozens of tiny maps, and a small tree falls below it and renders flat.
- `balance_ratio` stays `0.2`.

An empty or `None` tree falls back to `8` / `4`.
...
That simulator is public API in its own right — see
[Scenario simulation, coverage and A/B comparison](#scenario-simulation-coverage-and-ab-comparison)
for running the scenarios, not just screening them.
...
- The Scenarios view in
  [`web/streamlit_app_v6.py`](web/streamlit_app_v6.py) — no code needed.
  Its **Simulation** section runs and highlights the scenarios you just
  synthesized.

## Scenario simulation, coverage and A/B comparison

Synthesized scenarios are executable, so they can be **run** — offline,
under jUCMNav's own traversal semantics, without opening jUCMNav. A token
leaves each enabled start point; an OR-fork takes the branch whose guard
...
for result in pm4py_ucm.traverse_all(ucm):          # one per scenario
    status = "clean" if result.ok else "problems"
    print(f"{result.scenario}: {status}, {result.steps} steps")
    for problem in result.problems:
        print("   ", problem)
```

`traverse_all` raises `NoScenariosError` when the model declares none,
rather than reporting "no problems found" — a caller asking whether a model
executes cleanly must never be told *yes* on the strength of having run
nothing. `traverse_scenario(ucm, scenario)` runs a single one.

Each result reports the same four problem kinds jUCMNav's Problems view
does (blocked AND-join, unreached end point, no branch enabled, several
branches enabled), plus `peak_hit_count` — see
[the hit-count ceiling](#before-running-scenarios-in-jucmnav-check-the-hit-count-ceiling)
— and `visits`, the net hit count per element the run actually executed.

### What a scenario covered

`visits` is what makes coverage possible: it is keyed by `(type name, model
id)` rather than object identity, so a run can be compared against another,
serialised, or matched back onto a rendered diagram.

```python
results = pm4py_ucm.traverse_all(ucm)

cov = pm4py_ucm.coverage(ucm, results[:1])          # or any subset
print(cov.elements)          # (53, 97)   path nodes covered / total
print(cov.paths)             # (53, 108)  path segments covered / total
print(cov.by_kind())         # {'RespRef': (16, 28), 'OrFork': (6, 11), ...}
```

**Elements and paths are reported separately, not as one percentage.** A run
can walk every node and still miss segments — an alternative branch carrying
no responsibility *is* a segment and nothing else — so the split says whether
what is left uncovered is real behaviour or just empty paths. On
`ClaimsPaymentLog` (18 scenarios, all clean) a single scenario covers 53 of
97 elements; all 18 together reach 93/97 elements and 99/108 paths.

Coverage is measured against the **whole model**, every map included, which
is why one scenario reads low: it walks one path through a model that
contains every path. That is the honest number.

### Two scenarios, side by side

```python
cmp = pm4py_ucm.compare(ucm, results[0], results[1])
print(len(cmp.a_only), len(cmp.b_only), len(cmp.both))   # 7 25 99
print(f"{cmp.agreement:.0%}")                            # 76%
print(len(cmp.neither))                                  # never walked by either
```

`agreement` is the Jaccard index — the share of everything either scenario
walked that **both** walked. Two scenarios sharing a start point can never
score 0, so a very low number means they diverge almost immediately.

### Painting either onto the diagram

`coverage_render` and `comparison_render` turn a `Coverage` or a
`Comparison` into colours and hover text keyed by `id(element)`, ready for
the renderer:

```python
from pm4py_ucm.algo.scenario_coverage import comparison_render
from pm4py_ucm.visualization.ucm import svg

highlight = comparison_render(ucm, cmp)
open("compare.svg", "w", encoding="utf-8").write(
    svg.model_to_svg(ucm, "ucm", coverage=highlight))
```

A/B is painted **dark green** (A only), **dark orange** (B only) and
**purple** (both); the pair separates by lightness as well as hue, so it
survives a greyscale printout and does not lean on the red-green axis.
Supplying a highlight suspends the performance heat-map, since the two
compete for the same colour channel.

The web app's Scenarios view exposes all of this without code, and both the
saved project and the exported Python script carry the selection — see
[`docs/scenario_simulation.md`](docs/scenario_simulation.md) for the design
note behind it.
...
[`scenario_traversal`](#scenario-simulation-coverage-and-ab-comparison)'s
...
[`sessions.md`](docs/sessions.md) (what a saved project stores and how it is
kept from rotting) and [`code_export.md`](docs/code_export.md) (the registry →
