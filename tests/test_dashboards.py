...
class TestView:
    ...
    def test_payload_cannot_break_out_of_the_script_element(self):
        """An activity named `</script>` must not end the element."""
        df = _log()
    ...
    def test_document_has_exactly_the_scripts_it_should(self, table):
        """The real invariant: the document must not be truncated. Counting
        the tags catches a break-out that a substring check would miss."""
        html = dashboard_html(table)
        # One JSON data block + one module script, each opened and closed.
