...
**942 tests** across **28 modules**, grouped by area below. Counts are pytest-collected items (parametrised cases counted individually), so they match `pytest` exactly.
...
| | _Discovery & conversion subtotal_ | | **94** |
| **Resources & performers** | [`test_resources.py`](test_resources.py) | Tests for resource mining and performer binding. | 22 |
| **Scenario synthesis** | [`test_scenario_synthesis.py`](test_scenario_synthesis.py) | End-to-end smoke tests for variant clustering + scenario synthesis. | 43 |
...
|  | [`test_traversal.py`](test_traversal.py) | Tests for replay-based traversal counting. | 16 |
| | _Performance & metrics subtotal_ | | **78** |
