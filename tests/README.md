...
**956 tests** across **28 modules**, grouped by area below. Counts are pytest-collected items (parametrised cases counted individually), so they match `pytest` exactly.
...
|  | [`test_traversal.py`](test_traversal.py) | Tests for replay-based traversal counting. | 22 |
| | _Performance & metrics subtotal_ | | **84** |
